...
- **Custom Neural Models**: Deep learning models with overlapping fields, signature detection, and table/row/cell level confidence (NEW in v4.0)
...
- Custom classification model training up to **2 GB with 10,000 pages**
...
| Custom classification training | 2 GB max, 10,000 pages max |
...
| **Operational Excellence** | ⭐⭐⭐⭐⭐ | Azure Monitor integration, detailed metrics |
...
#### Recommended: Azure AI Speech (2025 Updates)
...
- **Semantic Segmentation** (NEW - January 2025): Integrates punctuation module inside decoder for more logical segmentation boundaries. 40%-60% reduction in longest segment latency
- **Fast Transcription API**: Supports multi-lingual transcription without specifying locale codes
...
- **May 2025**: Fast transcription accuracy improved 10-25% for major locales
- **April 2025**: Pronunciation assessment improvements for de-DE, es-MX, it-IT, ja-JP, ko-KR, pt-BR
- **January 2025**: Semantic Segmentation for en-US, en-GB, zh-CN, ja-JP, ko-KR, de-DE, fr-FR, it-IT, es-ES, es-MX, hi-IN, pt-BR, pt-PT, tr-TR, ru-RU, th-TH, id-ID
- **January 2025**: English model (en-US, en-CA) with 36% WER reduction for accessibility
...
| Polish | ✅ | ✅ | ✅ (NEW) | Full support |
| Urdu | ✅ | ✅ | ⚠️ | Via standard API |
...
2. **Fast transcription with diarisation** - For recorded audio files
3. **Batch transcription with diarisation** - For archive processing

**Note**: Conversation transcription multichannel diarisation was retired March 28, 2025. Use the above alternatives.
...
```python
# Example: Real-time conversation transcription with diarisation (2025 SDK)
...
# Enable semantic segmentation (NEW 2025)
speech_config.set_property(
...
| Pillar | Rating | Notes |
|--------|--------|-------|
| **Reliability** | ⭐⭐⭐⭐⭐ | 99.9% SLA, multi-region failover |
| **Security** | ⭐⭐⭐⭐⭐ | Audio data encrypted, biometric data protection |
| **Cost Optimization** | ⭐⭐⭐⭐☆ | Per-hour pricing, commitment discounts |
| **Operational Excellence** | ⭐⭐⭐⭐⭐ | Real-time metrics, pronunciation assessment |
| **Performance Efficiency** | ⭐⭐⭐⭐⭐ | <500ms latency meets NFR-P-002 |

**UK Region Availability**:
- ✅ UK South - Full feature availability including custom speech, fast transcription
...
```python
# Example: Speech-to-translated-text pipeline (2025)
...
#### Recommended: Azure AI Search (2025 Updates)

**Service Overview**:
- **Full Name**: Azure AI Search (formerly Azure Cognitive Search)
- **Category**: AI / Search
- **Current API**: 2025-09-01 (GA), 2025-11-01-preview
...
- **Agentic Retrieval** (NEW - May 2025): Conversational search experience powered by LLMs, breaks down complex queries into subqueries
- **Knowledge Bases** (NEW - November 2025): Create knowledge agents for downstream agent and chat model consumption
- **Query Rewrites** (Preview): Semantic ranker creates up to 10 query variants
- **Multivector Support** (May 2025): Index multiple child vectors within a single document field
- **AI Enrichment**: Built-in skills for OCR, entity extraction, key phrases
- **Document Layout Skill** (September 2025 GA): Integrated Document Intelligence processing
- **Security Trimming**: Respect document access controls in search results

**2025 Release Highlights**:
- **November 2025**: Semantic ranker and agentic retrieval on FREE tiers in select regions
- **September 2025**: Document Layout skill GA, OneLake indexer GA
- **May 2025**: Agentic retrieval preview, multivector support, scoring profiles with semantic ranking
- **March 2025**: Binary quantization rescoring, semantic ranker prerelease models
...
[Court Documents] --> [Azure Blob Storage] --> [AI Search Indexer]
                              ...
                              - Vector fields (multivector support)
                              - Security filters
```
...
**UK Region Availability**:
- ✅ UK South - All features including semantic ranker, agentic retrieval (free tier)
- ✅ UK West - All features available
- Free tier semantic ranker available in UK South (November 2025)
...
4. AI Search indexes content with AI enrichments (using Document Layout skill)
...
resource documentIntelligence 'Microsoft.CognitiveServices/accounts@2024-10-01' = {
  ...
  properties: {
    ...
    apiProperties: {
      // Enable v4.0 features
    }
  }
}

// Speech Service (2025 features)
...
3. **Free Tier Semantic Ranker** (NEW November 2025):
...
| Azure AI Speech (2025 Updates) | https://learn.microsoft.com/en-us/azure/ai-services/speech-service/ |
| Azure AI Translator | https://learn.microsoft.com/en-us/azure/ai-services/translator/ |
| Azure AI Search (2025 Updates) | https://learn.microsoft.com/en-us/azure/search/ |
...
| Speech SDK (2025) | https://github.com/Azure-Samples/cognitive-services-speech-sdk |
